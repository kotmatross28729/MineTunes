...
public abstract class AbstractID3v2Tag implements ID3v2 {
    ...
    private int unpackExtendedHeader(byte[] bytes, int offset) {
        extendedHeaderLength = BufferTools
            .unpackSynchsafeInteger(bytes[offset], bytes[offset + 1], bytes[offset + 2], bytes[offset + 3]) + 4;
    ...
    private int packSpecifiedFrames(byte[] bytes, int offset, String onlyId, String notId)
        ...
        Iterator<ID3v2FrameSet> setIterator = frameSets.values()
            .iterator();
    ...
    public String getTrack() {
        ...
        if (frameData != null && frameData.getText() != null) return frameData.getText()
            .toString();
        return null;
    }
    ...
    public boolean isCompilation() {
        ...
        if (frameData != null && frameData.getText() != null) return "1".equals(
            frameData.getText()
                .toString());
        return false;
    }
    ...
    public String getComment() {
        ID3v2CommentFrameData frameData = extractCommentFrameData(
            obseleteFormat ? ID_COMMENT_OBSELETE : ID_COMMENT,
            false);
        if (frameData != null && frameData.getComment() != null) return frameData.getComment()
            .toString();
        return null;
    }
    ...
    protected ID3v2TextFrameData extractTextFrameData(String id) {
        ID3v2FrameSet frameSet = frameSets.get(id);
        if (frameSet != null) {
            ID3v2Frame frame = (ID3v2Frame) frameSet.getFrames()
                .get(0);
    ...
    private ID3v2CommentFrameData extractCommentFrameData(String id, boolean itunes) {
        ID3v2FrameSet frameSet = frameSets.get(id);
        if (frameSet != null) {
            Iterator<ID3v2Frame> iterator = frameSet.getFrames()
                .iterator();
            while (iterator.hasNext()) {
                ...
                try {
                    ...
                    if (itunes && ITUNES_COMMENT_DESCRIPTION.equals(
                        frameData.getDescription()
                            .toString())) {
                        return frameData;
                    } else if (!itunes) {
                        return frameData;
