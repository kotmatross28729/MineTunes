...
public class GuiDevTools extends GuiMineTunes {
    ...
    public void initGui() {
        buttonList.clear();
        buttonList.add(
            new GuiButton(-1, width - 102, height - 22, 100, 20, StatCollector.translateToLocal("minetunes.gui.exit")));

        buttonList.add(new GuiButton(0, 10, 30, 200, 20, StatCollector.translateToLocal("minetunes.guidev.clear")));
        buttonList
            .add(new GuiButton(1, 10, 55, 200, 20, StatCollector.translateToLocal("minetunes.guidev.resetThread")));
        ...
        buttonList
            .add(new GuiButton(5, 10, 155, 200, 20, StatCollector.translateToLocal("minetunes.guidev.playPause")));
        buttonList
            .add(new GuiButton(6, 10, 180, 200, 20, StatCollector.translateToLocal("minetunes.guidev.volumeControl")));
        buttonList.add(
            new GuiButton(7, 10, 205, 200, 20, StatCollector.translateToLocal("minetunes.guidev.generatePlaylist")));
    ...
    public void drawScreen(int mx, int my, float partialTicks) {
        ...
        fontRendererObj
            .drawStringWithShadow(StatCollector.translateToLocal("minetunes.guidev.warning1"), 10, 5, 0xFFFFFF);
        fontRendererObj
            .drawStringWithShadow(StatCollector.translateToLocal("minetunes.guidev.warning2"), 10, 15, 0xFFFFFF);
    ...
    protected void actionPerformed(GuiButton button) {
        switch (button.id) {
            ...
            case 0:
                ...
                break;
            case 1:
                if (MineTunes.musicPlayerThread != null) MineTunes.musicPlayerThread.forceKill();
            ...
            case 6:
                if (MineTunes.musicPlayerThread != null) {
                    ...
                    debugLog(
                        "Audio Gain: " + gain
                            + " (Relative Volume: "
                            + MineTunes.musicPlayerThread.getRelativeVolume(gain)
                            + ")");
                }
                break;
            case 7:
                new FileSelector(
                    ...
                    ActionMakePlaylist.instance.withName(randomPlaylistName()));
                break;
        }
    }
    ...
    private String randomPlaylistName() {
        ...
        String name = "";
        for (int i = 0; i < 6; i++) name += chars[rand.nextInt(chars.length)];
    ...
    public static void debugLog(String s, boolean print) {
        if (MineTunes.DEBUG_MODE) {
            ...
            while (debugOut.size() > max) debugOut.remove(0);
        }
    }
