...
public abstract class PlaylistProvider {
    ...
    void updateState() {
        if (callback != null) callback.setState(generateProgressState());
    }
    ...
    String generateProgressState() {
        if (foundFiles > 0) {
            if (processedFiles > 0) {
                ...
                if (percent > 50) format = EnumChatFormatting.YELLOW;
                if (percent > 90) format = EnumChatFormatting.GREEN;

                return String.format(
                    StatCollector.translateToLocal("minetunes.playlist.generator.processing"),
                    processedFiles,
                    foundFiles) + format + " (" + percent + "%)";
            } else return String
                .format(StatCollector.translateToLocal("minetunes.playlist.generator.scanning"), foundFiles);
