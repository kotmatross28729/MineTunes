...
public final class HUDHandler {
    ...
    public void onDrawScreen(RenderGameOverlayEvent.Post event) {
        if (event.type == ElementType.ALL && MTConfig.hudEnabled) {
            ...
            if (MineTunes.musicPlayerThread != null && MineTunes.musicPlayerThread.player != null
                && MineTunes.musicPlayerThread.playingMP3 != null) {
                ...
                String volume = showVolume ? (String.format(
                    ...
                    "+" + (int) (MineTunes.musicPlayerThread.getRelativeVolume() * 100) + "%")) : "";
    ...
    private void renderSpectrumAnalyzer(Minecraft mc, int x, int y, int width, int height, int color, boolean invert) {
        ...
        for (int i = 0; i < SpectrumTools.BANDS; i++) {
            ...
            for (int j = 0; j < multiplier; j++) wFs += wFFT[i + j];
