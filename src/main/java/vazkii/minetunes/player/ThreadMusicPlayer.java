...
public class ThreadMusicPlayer extends Thread {
    ...
    public void pauseOrPlay() {
        if (player != null) if (!paused) {
            GuiDevTools.debugLog("Pausing...");
            paused = true;
            if (player.getAudioDevice() != null) player.stop();
        } else {
    ...
    public float getGain() {
        ...
        if (device != null && device instanceof JavaSoundAudioDevice) return ((JavaSoundAudioDevice) device).getGain();
        return gain;
    }
    ...
    public float getFractionPlayed() {
        ...
        if (pFrames == 0 || tFrames == 0) return 0F;
