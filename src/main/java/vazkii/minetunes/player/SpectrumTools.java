...
public final class SpectrumTools {
    ...
    private static float[] mergeStereoSamples(float[] sampleLeft, float[] sampleRight) {
        ...
        for (int i = 0; i < sampleLeft.length; i++) buffer[i] = (sampleLeft[i] + sampleRight[i]) * 0.5F;
    ...
    public static float[] getFFTCalculation() {
        ...
        if (sampleLeft == null) return null;

        if (decoder.getOutputChannels() == 2) sampleRight = decoder.getSamples(1);
