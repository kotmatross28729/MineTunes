...
public class Converter {
    ...
    public void convert(String sourceName, String destName, ProgressListener progressListener,
        ...
        if (destName.length() == 0) destName = null;
        try {
    ...
    public synchronized void convert(InputStream sourceStream, String destName, ProgressListener progressListener,
        ...
        try {
            if (!(sourceStream instanceof BufferedInputStream)) sourceStream = new BufferedInputStream(sourceStream);
            int frameCount = -1;
            ...
            if (frameCount == -1) frameCount = Integer.MAX_VALUE;
            ...
            try {
                for (; frame < frameCount; frame++) {
                    try {
                        ...
                        if (decoderOutput != output) throw new InternalError("Output buffers are different.");
